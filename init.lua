...
-- Buffer formatting utilities
vim.keymap.set("n", "=j", ":%!jq<CR>:set syntax=json<CR>", { silent = true, desc = "Format as JSON" })
vim.keymap.set("n", "=x", ":%!xmllint --format -<CR>:set syntax=xml<CR>", { silent = true, desc = "Format as XML" })
vim.keymap.set("n", "=s", ":%sort<CR>", { silent = true, desc = "Sort" })
...
nixInfo.lze.load({
  ...
  },
  {
    ...
    after = function(_)
      local function rust_map(buf)
        vim.keymap.set("n", "<leader>rr", function()
          vim.cmd.RustLsp("runnables")
        end, { buffer = buf, desc = "Rust: Runnables" })
        vim.keymap.set("n", "<leader>rd", function()
          vim.cmd.RustLsp("debuggables")
        end, { buffer = buf, desc = "Rust: Debuggables" })
        vim.keymap.set("n", "<leader>re", function()
          vim.cmd.RustLsp("expandMacro")
        end, { buffer = buf, desc = "Rust: Expand Macro" })
        vim.keymap.set("n", "<leader>rc", function()
          vim.cmd.RustLsp("openCargo")
        end, { buffer = buf, desc = "Rust: Open Cargo.toml" })
        vim.keymap.set("n", "K", function()
          vim.cmd.RustLsp({ "hover", "actions" })
        end, { buffer = buf, desc = "Rust: Hover Actions" })
      end
      rust_map(vim.api.nvim_get_current_buf())
      vim.api.nvim_create_autocmd("FileType", {
        pattern = "rust",
        callback = function(args)
          rust_map(args.buf)
